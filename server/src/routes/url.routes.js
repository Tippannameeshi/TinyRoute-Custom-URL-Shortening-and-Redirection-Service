...
const { createUrlValidator, updateUrlValidator } = require("../validators/url.validator");
...
router.get("/:id/qr", asyncHandler(async (req, res) => {
  ...
  const targetShortUrl = buildShortUrl(urlRecord.short_code, urlRecord.custom_alias);

  if (format === 'svg') {
    const svgString = await QrService.generateSvgString(targetShortUrl);
    res.setHeader('Content-Type', 'image/svg+xml');
    res.setHeader('Content-Disposition', `attachment; filename="qr_${urlRecord.short_code}.svg"`);
    return res.send(svgString);
  }

  const dataUrl = await QrService.generateDataUrl(targetShortUrl);
  return ApiResponse.success(res, HTTP_STATUS.OK, "QR code generated successfully", { dataUrl, shortUrl: targetShortUrl });
}));
